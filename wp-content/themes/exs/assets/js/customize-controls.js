...
(function ($, api) {
	...
	api.bind('ready', function () {
		...
		colors.forEach(function (color) {
			api(color, function (control) {
				control.bind(function () {
					...
					document.documentElement.style.setProperty('--'+color, control.get());
		...
		//mobile menu
		api('mobile_nav_width', function (control) {
			control.bind(function () {
				if (!control) {
					return;
				}
				var val = control.get();
				if (val) {
					previewWrap.firstChild.contentWindow.document.documentElement.style.setProperty('--mobileNavWidth', val+'px');
				}
			});
		});
		//mobile menu mobile_nav_px
		api('mobile_nav_px', function (control) {
			control.bind(function () {
				if (!control) {
					return;
				}
				var val = control.get();
				if (val) {
					previewWrap.firstChild.contentWindow.document.documentElement.style.setProperty('--mobileNavPX', val+'px');
				}
			});
		});
		//button font size
		api('buttons_fs', function (control) {
			control.bind(function () {
				...
				var val = control.get();
				if (val) {
					previewWrap.firstChild.contentWindow.document.documentElement.style.setProperty('--btn-fs', val+'px');
				} else {
					previewWrap.firstChild.contentWindow.document.documentElement.style.setProperty('--btn-fs', '.92em');
				}
			});
		});
		...
		api('main_gap_width', function (control) {
			control.bind(function () {
				...
				var val = control.get();
				if (val) {
					previewWrap.firstChild.contentWindow.document.documentElement.style.setProperty('--sb-gap', val+'rem');
				} else {
					previewWrap.firstChild.contentWindow.document.documentElement.style.setProperty('--sb-gap', '2.5rem');
				}
			});
		});

		////////////////////////////////////
		//buttons,menu,fullwidth thumbnail//
		////////////////////////////////////
			//checkboxes
			// 'buttons_uppercase'
			// 'buttons_bold'
			// 'buttons_colormain'
			// 'buttons_outline'
			// layout - post_thumbnails_fullwidth
		...
		function switchCSSfile(controlName,cssLinkID,cssFilePath) {
			api(controlName, function (control) {
				control.bind(function () {
					if (!control) {
						return;
					}
					var style = previewWrap.firstChild.contentWindow.document.getElementById(cssLinkID);
					if( ! style) {
						return;
					}
					var val = control.get();
					if (control.get()) {
						style.setAttribute('href',exsCustomizerObject.themeUrl+cssFilePath+val+'.css');
					} else {
						style.setAttribute('href',exsCustomizerObject.themeUrl+cssFilePath+'0.css');
					}
				});
			});
		}

		switchCSSfile('menu_desktop','exs-menu-desktop-type-style-css','/assets/css/min/menu-desktop');
		switchCSSfile('button_burger','exs-burger-type-style-css','/assets/css/min/burger-type');
		switchCSSfile('buttons_pagination','exs-pagination-type-style-css','/assets/css/min/pagination-type');
		switchCSSfile('skin','exs-skin-css','/extra/assets/css/min/skin');
